...
export class EntityRequest<A = any, B = any, C = A[]> {
    ...
    Functions() {
        const create = (body: A, locale: string, Authorization = '') => YouthRoute<{ id: string }>('post', this.url, {}, { Authorization, locale }, body, 1000 * 60 * 13);
        const update = (id: string, body: B, locale: string, Authorization = '') => YouthRoute<{ msg: "UPDATED" }>('patch', `${this.url}/${id}`, {}, { Authorization, locale }, body, 1000 * 60 * 13);
    ...
    CategoryFunctions() {
        const create = (body: Category, locale: string, Authorization = '') => YouthRoute<{ id: string }>('post', `${this.url}-category`, {}, { Authorization, locale }, body, 1000 * 60 * 13);
        const update = (id: string, body: CategoryOption, locale: string, Authorization = '') => YouthRoute<{ msg: "UPDATED" }>('patch', `${this.url}-category/${id}`, {}, { Authorization, locale }, body, 1000 * 60 * 13);
